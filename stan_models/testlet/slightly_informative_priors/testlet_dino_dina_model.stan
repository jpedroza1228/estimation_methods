data {
  ...
  int<lower=1> K;
  int<lower=1> T;
  vector<lower=1,upper=T>[T] testlet; // T for testlet
...
parameters {
  ...
  vector<lower=0>[T] testlet_var;
  matrix[J, T] gamma; // testlet effect on each person
}
...
transformed parameters {
  ...
  for (j in 1:J){
    for (t in 1:T) {
    gamma_up[j,t] = gamma[j,t] * testlet_var[t];
  }
  }
}
...
model{
  ...
  for (j in 1:J){
    for (t in 1:T){
      gamma[j,t] ~ normal(0, 1);
    }
  }
  testlet_var ~ exponential(1);
